...
static void b64_initialize_rmap() {
  ...
  for (i = 1; i < 256; ++i) {
    ch = (unsigned char)i;
    /* Whitespaces */
    if (isspace(ch)) b64rmap[i] = b64rmap_space;
  ...
  for (i = 0; Base64[i] != '\0'; ++i) b64rmap[(uint8_t)Base64[i]] = i;
...
static int b64_pton_do(char const *src, uint8_t *target, size_t targsize) {
  ...
  while (1) {
    ...
    if (ofs >= b64rmap_special) {
      /* Ignore whitespaces */
      if (ofs == b64rmap_space) continue;
      /* End of base64 characters */
      if (ofs == b64rmap_end) break;
    ...
    switch (state) {
      ...
      case 2: /* Valid, means one byte of info */
        ...
        for ((void)NULL; ch != '\0'; ch = *src++)
          if (b64rmap[ch] != b64rmap_space) break;
      ...
      case 3: /* Valid, means two bytes of info */
         ...
         */
        for ((void)NULL; ch != '\0'; ch = *src++)
          if (b64rmap[ch] != b64rmap_space) return (-1);
...
static int b64_pton(char const *src, uint8_t *target, size_t targsize) {
  if (!b64rmap_initialized) b64_initialize_rmap();
