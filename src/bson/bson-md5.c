...
static void bson_md5_process(bson_md5_t *md5, const uint8_t *data) {
...
void bson_md5_append(bson_md5_t *pms, const uint8_t *data, uint32_t nbytes) {
  ...
  /* Process full blocks. */
  for (; left >= 64; p += 64, left -= 64) bson_md5_process(pms, p);

  /* Process a final partial block. */
  if (left) memcpy(pms->buf, p, left);
}

void bson_md5_finish(bson_md5_t *pms, uint8_t digest[16]) {
