...
bson_string_t *bson_string_new(const char *str) /* IN */
{
  bson_string_t *ret;
...
char *bson_string_free(bson_string_t *string, /* IN */
...
void bson_string_append(bson_string_t *string, /* IN */
                        const char *str)       /* IN */
{
  uint32_t len;
...
void bson_string_append_c(bson_string_t *string, /* IN */
                          char c)                /* IN */
{
  char cc[2];
...
void bson_string_append_unichar(bson_string_t *string,  /* IN */
...
void bson_string_truncate(bson_string_t *string, /* IN */
                          uint32_t len)          /* IN */
{
  uint32_t alloc;
...
char *bson_strdup(const char *str) /* IN */
{
  long len;
  char *out;
...
char *bson_strdupv_printf(const char *format, /* IN */
                          va_list args)       /* IN */
{
  va_list my_args;
  char *buf;
...
char *bson_strdup_printf(const char *format, /* IN */
                         ...)                /* IN */
{
  va_list args;
  char *ret;
...
char *bson_strndup(const char *str, /* IN */
                   size_t n_bytes)  /* IN */
{
  char *ret;
...
void bson_strfreev(char **str) /* IN */
{
  int i;

  if (str) {
    for (i = 0; str[i]; i++) bson_free(str[i]);
    bson_free(str);
  }
}
...
size_t bson_strnlen(const char *s, /* IN */
                    size_t maxlen) /* IN */
{
...
int bson_vsnprintf(char *str,          /* IN */
                   size_t size,        /* IN */
...
int64_t bson_ascii_strtoll(const char *s, char **e, int base) {
  char *tok = (char *)s;
  char *digits_start;
  char c;
  int64_t number = 0;
...
int bson_strcasecmp(const char *s1, const char *s2) {
#ifdef BSON_OS_WIN32
