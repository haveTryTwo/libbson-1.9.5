...
static bool get_tok(const char *terminals, const char **ptr, int32_t *remaining, const char **out,
                    int32_t *out_len) {
  const char *terminal;
...
static bool digits_only(const char *str, int32_t len) {
  int i;
...
static bool parse_num(const char *str, int32_t len, int32_t digits, int32_t min, int32_t max,
                      int32_t *out) {
  int i;
  int magnitude = 1;
  int32_t value = 0;
...
bool _bson_iso8601_date_parse(const char *str, int32_t len, int64_t *out, bson_error_t *error) {
  const char *ptr;
...
#define DATE_PARSE_ERR(msg)                                                  \
  bson_set_error(error, BSON_ERROR_JSON, BSON_JSON_ERROR_READ_INVALID_PARAM, \
                 "Could not parse \"%s\" as date: " msg, str);               \
  return false

#define DEFAULT_DATE_PARSE_ERR                                    \
  DATE_PARSE_ERR(                                                 \
      "use ISO8601 format yyyy-mm-ddThh:mm plus timezone, either" \
      " \"Z\" or like \"+0500\"")
...
void _bson_iso8601_date_format(int64_t msec_since_epoch, bson_string_t *str) {
  time_t t;
  int64_t msecs_part;
  char buf[64];
