...
static void _bson_json_code_cleanup(bson_json_code_t *code_data) {
...
/* forward decl */
static void _bson_json_save_map_key(bson_json_reader_bson_t *bson, const uint8_t *val, size_t len);
...
#define BASIC_CB_BAIL_IF_NOT_NORMAL(_type)                                                   \
  if (bson->read_state != BSON_JSON_REGULAR) {                                               \
    _bson_json_read_set_error(reader, "Invalid read of %s in state %s", (_type),             \
                              read_state_names[bson->read_state]);                           \
    return;                                                                                  \
  } else if (!key) {                                                                         \
    _bson_json_read_set_error(reader, "Invalid read of %s without key in state %s", (_type), \
                              read_state_names[bson->read_state]);                           \
    return;                                                                                  \
  }
  ...
  (len == strlen(_key) && strncmp((const char *)val, (_key), len) == 0) {      \
    if (bson->bson_type && bson->bson_type != (_type)) {                       \
      _bson_json_read_set_error(reader,                                        \
                                ...
                                "for type \"%s\", got \"%s\"",                 \
                                (_key), _bson_json_type_name(bson->bson_type), \
...
static void _bson_json_read_set_error(bson_json_reader_t *reader, const char *fmt, ...)
    BSON_GNUC_PRINTF(2, 3);

static void _bson_json_read_set_error(bson_json_reader_t *reader, /* IN */
                                      const char *fmt,            /* IN */
                                      ...) {
  va_list ap;
...
static void _bson_json_read_corrupt(bson_json_reader_t *reader, const char *fmt, ...)
    BSON_GNUC_PRINTF(2, 3);

static void _bson_json_read_corrupt(bson_json_reader_t *reader, /* IN */
                                    const char *fmt,            /* IN */
                                    ...) {
  va_list ap;
...
static void _bson_json_buf_ensure(bson_json_buf_t *buf, /* IN */
                                  size_t len)           /* IN */
{
  if (buf->n_bytes < len) {
    bson_free(buf->buf);
...
static void _bson_json_buf_set(bson_json_buf_t *buf, const void *from, size_t len) {
...
static void _bson_json_buf_append(bson_json_buf_t *buf, const void *from, size_t len) {
...
static const char *_bson_json_type_name(bson_type_t type) {
  switch (type) {
    case BSON_TYPE_EOD:
...
static void _bson_json_read_fixup_key(bson_json_reader_bson_t *bson) /* IN */
{
...
static void _bson_json_read_null(bson_json_reader_t *reader) {
  BASIC_CB_PREAMBLE;
...
static void _bson_json_read_boolean(bson_json_reader_t *reader, /* IN */
                                    int val)                    /* IN */
{
  BASIC_CB_PREAMBLE;
...
/* sign is -1 or 1 */
static void _bson_json_read_integer(bson_json_reader_t *reader, uint64_t val, int64_t sign) {
  ...
  if (rs == BSON_JSON_REGULAR) {
    ...
    switch (bs) {
      ...
      default:
        _bson_json_read_set_error(reader, "Unexpected integer %s%" PRIu64 " in type \"%s\"",
                                  sign == -1 ? "-" : "", val,
                                  _bson_json_type_name(bson->bson_type));
    }
  } else {
    _bson_json_read_set_error(reader, "Unexpected integer %s%" PRIu64 " in state \"%s\"",
                              sign == -1 ? "-" : "", val, read_state_names[rs]);
  }
}

static bool _bson_json_parse_double(bson_json_reader_t *reader, const char *val, size_t vlen,
                                    double *d) {
  errno = 0;
  *d = strtod(val, NULL);
  ...
  if ((*d == HUGE_VAL || *d == -HUGE_VAL) && errno == ERANGE &&
      strncasecmp(val, "infinity", vlen) && strncasecmp(val, "-infinity", vlen)) {
    _bson_json_read_set_error(reader, "Number \"%.*s\" is out of range", (int)vlen, val);
...
static void _bson_json_read_double(bson_json_reader_t *reader, /* IN */
                                   double val)                 /* IN */
{
  BASIC_CB_PREAMBLE;
...
static bool _bson_json_read_int64_or_set_error(bson_json_reader_t *reader, /* IN */
...
static void _bson_json_parse_binary_elem(bson_json_reader_t *reader, const char *val_w_null,
                                         size_t vlen) {
  bson_json_read_bson_state_t bs;
  ...
  if (bs == BSON_JSON_LF_BINARY) {
    ...
    b64_pton(val_w_null, bson->bson_type_buf[0].buf, (size_t)binary_len + 1);
    ...
    if (SSCANF(val_w_null, "%02x", &data->binary.type) != 1) {
      if (!data->binary.is_legacy || data->binary.has_binary) {
        ...
        _bson_json_read_set_error(reader, "Invalid input string \"%s\", looking for binary subtype",
                                  val_w_null);
      } else {
        /* actually a query operator: {x: {$type: "array"}}*/
        bson->read_state = BSON_JSON_REGULAR;
        STACK_PUSH_DOC(
            bson_append_document_begin(STACK_BSON_PARENT, key, (int)len, STACK_BSON_CHILD));
...
static void _bson_json_read_string(bson_json_reader_t *reader, /* IN */
                                   const unsigned char *val,   /* IN */
                                   size_t vlen)                /* IN */
{
  bson_json_read_state_t rs;
  ...
  if (rs == BSON_JSON_REGULAR) {
    ...
    _bson_json_read_set_error(reader, "Invalid read of \"%s\" in state \"%s\"", val,
                              read_state_names[rs]);
    ...
    switch (bs) {
      ...
      case BSON_JSON_LF_DOUBLE: {
        _bson_json_parse_double(reader, (const char *)val, vlen,
    ...
    return;
  BAD_PARSE:
    _bson_json_read_set_error(reader, "Invalid input string \"%s\", looking for %s", val_w_null,
                              bson_state_names[bs]);
  } else {
...
static void _bson_json_read_start_map(bson_json_reader_t *reader) /* IN */
{
  BASIC_CB_PREAMBLE;
...
static bool _is_known_key(const char *key, size_t len) {
  bool ret;

#define IS_KEY(k) (len == strlen(k) && (0 == memcmp(k, key, len)))

  ret = (IS_KEY("$regularExpression") || IS_KEY("$regex") || IS_KEY("$options") ||
         IS_KEY("$code") || IS_KEY("$scope") || IS_KEY("$oid") || IS_KEY("$binary") ||
         IS_KEY("$type") || IS_KEY("$date") || IS_KEY("$undefined") || IS_KEY("$maxKey") ||
         IS_KEY("$minKey") || IS_KEY("$timestamp") || IS_KEY("$numberInt") ||
         IS_KEY("$numberLong") || IS_KEY("$numberDouble") || IS_KEY("$numberDecimal") ||
         IS_KEY("$numberInt") || IS_KEY("$numberLong") || IS_KEY("$numberDouble") ||
         IS_KEY("$numberDecimal") || IS_KEY("$dbPointer") || IS_KEY("$symbol"));
...
static void _bson_json_read_code_or_scope_key(bson_json_reader_bson_t *bson, bool is_scope,
  ...
  if (code->in_scope) {
    ...
    bson->read_state = BSON_JSON_REGULAR;
    STACK_PUSH_DOC(bson_append_document_begin(STACK_BSON_PARENT, bson->key, (int)bson->key_buf.len,
                                              STACK_BSON_CHILD));
...
static void _bson_json_bad_key_in_type(bson_json_reader_t *reader, /* IN */
                                       const uint8_t *val)         /* IN */
{
  bson_json_reader_bson_t *bson = &reader->bson;

  _bson_json_read_set_error(reader, "Invalid key \"%s\".  Looking for values for type \"%s\"", val,
                            _bson_json_type_name(bson->bson_type));
}

static void _bson_json_read_map_key(bson_json_reader_t *reader, /* IN */
                                    const uint8_t *val,         /* IN */
                                    size_t len)                 /* IN */
{
  bson_json_reader_bson_t *bson = &reader->bson;
  ...
  if (bson->read_state == BSON_JSON_IN_START_MAP) {
    if (len > 0 && val[0] == '$' && _is_known_key((const char *)val, len) &&
      ...
      STACK_PUSH_DOC(bson_append_document_begin(STACK_BSON_PARENT, bson->key,
                                                (int)bson->key_buf.len, STACK_BSON_CHILD));
    }
  ...
  if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
    ...
    if HANDLE_OPTION ("t", BSON_TYPE_TIMESTAMP, BSON_JSON_LF_TIMESTAMP_T)
      else if HANDLE_OPTION ("i", BSON_TYPE_TIMESTAMP, BSON_JSON_LF_TIMESTAMP_I) else {
        _bson_json_bad_key_in_type(reader, val);
      }
  } else if (bson->read_state == BSON_JSON_IN_BSON_TYPE_REGEX_VALUES) {
    if HANDLE_OPTION ("pattern", BSON_TYPE_REGEX, BSON_JSON_LF_REGULAR_EXPRESSION_PATTERN)
      else if HANDLE_OPTION ("options", BSON_TYPE_REGEX,
    ...
    if HANDLE_OPTION ("base64", BSON_TYPE_BINARY, BSON_JSON_LF_BINARY)
      else if HANDLE_OPTION ("subType", BSON_TYPE_BINARY, BSON_JSON_LF_TYPE) else {
        _bson_json_bad_key_in_type(reader, val);
      }
  } else {
    _bson_json_save_map_key(bson, val, len);
...
static void _bson_json_read_append_binary(bson_json_reader_t *reader,    /* IN */
                                          bson_json_reader_bson_t *bson) /* IN */
{
  ...
  if (!bson_append_binary(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len, data->binary.type,
                          bson->bson_type_buf[0].buf, (uint32_t)bson->bson_type_buf[0].len)) {
    _bson_json_read_set_error(reader, "Error storing binary data");
  }
}

static void _bson_json_read_append_regex(bson_json_reader_t *reader,    /* IN */
                                         bson_json_reader_bson_t *bson) /* IN */
{
  bson_json_bson_data_t *data = &bson->bson_type_data;
  if (data->regex.is_legacy) {
    ...
    _bson_json_read_set_error(reader,
                              "Missing \"pattern\" after \"options\" in regular expression");
    return;
  } else if (!data->regex.has_options) {
    _bson_json_read_set_error(reader,
                              "Missing \"options\" after \"pattern\" in regular expression");
    return;
  }

  if (!bson_append_regex(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                         (char *)bson->bson_type_buf[0].buf, (char *)bson->bson_type_buf[1].buf)) {
    _bson_json_read_set_error(reader, "Error storing regex");
  }
}

static void _bson_json_read_append_code(bson_json_reader_t *reader,    /* IN */
                                        bson_json_reader_bson_t *bson) /* IN */
{
  bson_json_code_t *code_data;
  char *code = NULL;
  ...
  r = bson_append_code_with_scope(STACK_BSON_CHILD, (const char *)code_data->key_buf.buf,
...
static void _bson_json_read_append_dbpointer(bson_json_reader_t *reader,    /* IN */
                                             bson_json_reader_bson_t *bson) /* IN */
{
  bson_t *db_pointer;
  bson_iter_t iter;
  ...
  while (bson_iter_next(&iter)) {
    if (!strcmp(bson_iter_key(&iter), "$id")) {
      ...
      _bson_json_read_set_error(reader, "$dbPointer contains invalid key: \"%s\"",
                                bson_iter_key(&iter));
      return;
    }
  }
  ...
  r = bson_append_dbpointer(STACK_BSON_CHILD, (char *)reader->bson.dbpointer_key.buf,
                            (int)reader->bson.dbpointer_key.len, ns, oid);
...
static void _bson_json_read_append_oid(bson_json_reader_t *reader,    /* IN */
                                       bson_json_reader_bson_t *bson) /* IN */
{
  if (!bson_append_oid(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                       &bson->bson_type_data.oid.oid)) {
    _bson_json_read_set_error(reader, "Error storing ObjectId");
  }
}

static void _bson_json_read_append_date_time(bson_json_reader_t *reader,    /* IN */
                                             bson_json_reader_bson_t *bson) /* IN */
{
  if (!bson_append_date_time(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                             bson->bson_type_data.date.date)) {
    _bson_json_read_set_error(reader, "Error storing datetime");
  }
}

static void _bson_json_read_append_timestamp(bson_json_reader_t *reader,    /* IN */
                                             bson_json_reader_bson_t *bson) /* IN */
{
  ...
  bson_append_timestamp(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                        bson->bson_type_data.timestamp.t, bson->bson_type_data.timestamp.i);
}

static void _bad_extended_json(bson_json_reader_t *reader) {
  _bson_json_read_corrupt(reader, "Invalid MongoDB extended JSON");
}

static void _bson_json_read_end_map(bson_json_reader_t *reader) /* IN */
{
  bson_json_reader_bson_t *bson = &reader->bson;
  ...
  if (bson->read_state == BSON_JSON_IN_BSON_TYPE) {
    ...
    switch (bson->bson_type) {
      ...
      case BSON_TYPE_DATE_TIME:
        ...
        break;
      case BSON_TYPE_UNDEFINED:
        bson_append_undefined(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len);
        break;
      case BSON_TYPE_MINKEY:
        bson_append_minkey(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len);
        break;
      case BSON_TYPE_MAXKEY:
        bson_append_maxkey(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len);
        break;
      case BSON_TYPE_INT32:
        bson_append_int32(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                          bson->bson_type_data.v_int32.value);
        break;
      case BSON_TYPE_INT64:
        bson_append_int64(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                          bson->bson_type_data.v_int64.value);
        break;
      case BSON_TYPE_DOUBLE:
        bson_append_double(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
                           bson->bson_type_data.v_double.value);
        break;
      case BSON_TYPE_DECIMAL128:
        bson_append_decimal128(STACK_BSON_CHILD, bson->key, (int)bson->key_buf.len,
    ...
    if (STACK_IS_SCOPE) {
      ...
      if (STACK_HAS_ID != STACK_HAS_REF) {
        _bson_json_read_set_error(reader, "%s", "DBRef object must have both $ref and $id keys");
      }
...
static void _bson_json_read_start_array(bson_json_reader_t *reader) /* IN */
{
  const char *key;
  size_t len;
  bson_json_reader_bson_t *bson = &reader->bson;
  ...
  if (bson->read_state != BSON_JSON_REGULAR) {
    _bson_json_read_set_error(reader, "Invalid read of \"[\" in state \"%s\"",
                              read_state_names[bson->read_state]);
    return;
  }
...
static void _bson_json_read_end_array(bson_json_reader_t *reader) /* IN */
{
  bson_json_reader_bson_t *bson = &reader->bson;
...
static bool _bson_json_unescape(bson_json_reader_t *reader, struct jsonsl_state_st *state,
                                const char *json_text, ssize_t len) {
  ...
  if (err != JSONSL_ERROR_SUCCESS) {
    bson_set_error(reader->error, BSON_ERROR_JSON, BSON_JSON_ERROR_READ_CORRUPT_JS,
                   "error near position %d: \"%s\"", (int)state->pos_begin, jsonsl_strerror(err));
    return false;
  }
...
static const char *_get_json_text(jsonsl_t json,                 /* IN */
                                  struct jsonsl_state_st *state, /* IN */
                                  const char *buf /* IN */, ssize_t *len /* OUT */) {
...
static void _push_callback(jsonsl_t json, jsonsl_action_t action, struct jsonsl_state_st *state,
                           const char *buf) {
...
static void _pop_callback(jsonsl_t json, jsonsl_action_t action, struct jsonsl_state_st *state,
                          const char *buf) {
  bson_json_reader_t *reader;
  bson_json_reader_bson_t *reader_bson;
  ssize_t len;
  ...
  switch (state->type) {
    ...
    case JSONSL_T_SPECIAL:
      ...
      if (state->special_flags & JSONSL_SPECIALf_NUMNOINT) {
        ...
        _bson_json_read_integer(reader, state->nelem,
...
static int _error_callback(jsonsl_t json, jsonsl_error_t err, struct jsonsl_state_st *state,
                           char *errat) {
  bson_json_reader_t *reader = (bson_json_reader_t *)json->data;
  ...
  bson_set_error(reader->error, BSON_ERROR_JSON, BSON_JSON_ERROR_READ_CORRUPT_JS,
                 "Got parse error at \"%c\", position %d: \"%s\"", *errat, (int)json->pos,
                 jsonsl_strerror(err));
...
int bson_json_reader_read(bson_json_reader_t *reader, /* IN */
                          bson_t *bson,               /* IN */
...
bson_json_reader_t *bson_json_reader_new(void *data,               /* IN */
...
void bson_json_reader_destroy(bson_json_reader_t *reader) /* IN */
{
  int i;
...
static ssize_t _bson_json_data_reader_cb(void *_ctx, uint8_t *buf, size_t len) {
  size_t bytes;
...
bson_json_reader_t *bson_json_data_reader_new(bool allow_multiple, /* IN */
                                              size_t size)         /* IN */
{
...
void bson_json_data_reader_ingest(bson_json_reader_t *reader, /* IN */
...
bson_t *bson_new_from_json(const uint8_t *data, /* IN */
                           ssize_t len,         /* IN */
                           bson_error_t *error) /* OUT */
{
  bson_json_reader_t *reader;
...
bool bson_init_from_json(bson_t *bson,        /* OUT */
                         const char *data,    /* IN */
                         ssize_t len,         /* IN */
                         bson_error_t *error) /* OUT */
{
...
static void _bson_json_reader_handle_fd_destroy(void *handle) /* IN */
{
...
static ssize_t _bson_json_reader_handle_fd_read(void *handle, /* IN */
                                                uint8_t *buf, /* IN */
                                                size_t len)   /* IN */
{
  bson_json_reader_handle_fd_t *fd = handle;
...
bson_json_reader_t *bson_json_reader_new_from_fd(int fd,                /* IN */
                                                 bool close_on_destroy) /* IN */
{
  ...
  return bson_json_reader_new(handle, _bson_json_reader_handle_fd_read,
                              _bson_json_reader_handle_fd_destroy, true,
                              BSON_JSON_DEFAULT_BUF_SIZE);
}

bson_json_reader_t *bson_json_reader_new_from_file(const char *path,    /* IN */
                                                   bson_error_t *error) /* OUT */
{
