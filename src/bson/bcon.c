...
const char *bson_bcon_magic(void) { return gBconMagic; }

const char *bson_bcone_magic(void) { return gBconeMagic; }

static void _noop(void) {}

/* appends val to the passed bson object.  Meant to be a super simple dispatch
 * table */
static void _bcon_append_single(bson_t *bson, bcon_type_t type, const char *key,
                                bcon_append_t *val) {
  switch ((int)type) {
    case BCON_TYPE_UTF8:
    ...
    case BCON_TYPE_DOUBLE:
      ...
      break;
    case BCON_TYPE_BIN: {
      bson_append_binary(bson, key, -1, val->BIN.subtype, val->BIN.binary, val->BIN.length);
      break;
    }
    ...
    case BCON_TYPE_DBPOINTER: {
      bson_append_dbpointer(bson, key, -1, val->DBPOINTER.collection, val->DBPOINTER.oid);
      break;
    }
    case BCON_TYPE_CODE:
    ...
    case BCON_TYPE_CODEWSCOPE:
      bson_append_code_with_scope(bson, key, -1, val->CODEWSCOPE.js, val->CODEWSCOPE.scope);
      break;
    case BCON_TYPE_INT32:
      bson_append_int32(bson, key, -1, val->INT32);
      break;
    case BCON_TYPE_TIMESTAMP: {
      bson_append_timestamp(bson, key, -1, val->TIMESTAMP.timestamp, val->TIMESTAMP.increment);
      break;
    }
 ...
 * */
static bool _bcon_extract_single(const bson_iter_t *iter, bcon_type_t type, bcon_extract_t *val) {
  switch ((int)type) {
    case BCON_TYPE_UTF8:
 ...
 */
static bcon_type_t _bcon_append_tokenize(va_list *ap, bcon_append_t *u) {
  char *mark;
  bcon_type_t type;
 ...
 *       II. If not, just call it a UTF8 token and pass that back
 */
static bcon_type_t _bcon_extract_tokenize(va_list *ap, bcon_extract_t *u) {
  char *mark;
  bcon_type_t type;
...
static void _bson_concat_array(bson_t *dest, const bson_t *src, bcon_append_ctx_t *ctx) {
  bson_iter_t iter;
  const char *key;
  char i_str[16];
  bool r;
 ...
 * defined up top.
 * */
void bcon_append_ctx_va(bson_t *bson, bcon_append_ctx_t *ctx, va_list *ap) {
  bcon_type_t type;
  const char *key;
  char i_str[16];
