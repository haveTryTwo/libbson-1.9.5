 ...
 */
static BSON_INLINE uint32_t bson_iter_utf8_len_unsafe(const bson_iter_t *iter) {
  int32_t val;
...
BSON_EXPORT(void)
bson_iter_binary(const bson_iter_t *iter, bson_subtype_t *subtype, uint32_t *binary_len,
 ...
 */
static BSON_INLINE const char *bson_iter_code_unsafe(const bson_iter_t *iter, uint32_t *length) {
  *length = bson_iter_utf8_len_unsafe(iter);
  return (const char *)(iter->raw + iter->d2);
}

BSON_EXPORT(const char *)
bson_iter_codewscope(const bson_iter_t *iter, uint32_t *length, uint32_t *scope_len,
                     const uint8_t **scope);

BSON_EXPORT(void)
bson_iter_dbpointer(const bson_iter_t *iter, uint32_t *collection_len, const char **collection,
 ...
 * Returns: A double.
 */
static BSON_INLINE double bson_iter_double_unsafe(const bson_iter_t *iter) {
  double val;
 ...
 */
static BSON_INLINE int32_t bson_iter_int32_unsafe(const bson_iter_t *iter) {
  int32_t val;
 ...
 */
static BSON_INLINE int64_t bson_iter_int64_unsafe(const bson_iter_t *iter) {
  int64_t val;
 ...
 */
static BSON_INLINE const bson_oid_t *bson_iter_oid_unsafe(const bson_iter_t *iter) {
 ...
 */
static BSON_INLINE void bson_iter_decimal128_unsafe(const bson_iter_t *iter,
 ...
 * Returns: A string that should not be modified or freed.
 */
static BSON_INLINE const char *bson_iter_key_unsafe(const bson_iter_t *iter) {
 ...
 * Returns: A string that should not be modified or freed.
 */
static BSON_INLINE const char *bson_iter_utf8_unsafe(const bson_iter_t *iter, size_t *length) {
  *length = bson_iter_utf8_len_unsafe(iter);
  return (const char *)(iter->raw + iter->d2);
}
 ...
 *          in UTC.
 */
static BSON_INLINE time_t bson_iter_time_t_unsafe(const bson_iter_t *iter) {
 ...
 */
static BSON_INLINE void bson_iter_timeval_unsafe(const bson_iter_t *iter, struct timeval *tv) {
 ...
 */
static BSON_INLINE bool bson_iter_bool_unsafe(const bson_iter_t *iter) {
  char val;
 ...
 */
static BSON_INLINE bson_type_t bson_iter_type_unsafe(const bson_iter_t *iter) {
